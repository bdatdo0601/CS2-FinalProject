	...
	**/	
	...
	public static double[][] rowReduce(double [][] m)
	{
	    ...
	    for(int r = 0; r < row && !end; r++)
	    {
	        ...
	        while(!end && rowReduced[rowOn][pivotNum] == 0) //Checking the numbers going down the column, if that number is 0, continue down
	        {
	        ...
	        if(!end && rowReduced[rowOn][pivotNum] != 0) //goes here if that index from above isn't 0
	        {
	            ...
	            {
	                multiplyRow(m, r, (1.0 / rowReduced[r][pivotNum])); //Multiply CURRENT row by scalar multiple 
	            }
