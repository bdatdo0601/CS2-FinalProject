...
public class MultiplyMatrix 
{
	public static int[][] multiply(int[][] a, int[][] b) 
	{
		...
		int[][] C = new int[rowsInA][columnsInB]; //stores results in new matrix (dimensions = outer dimensions of A * B)
		...
		for (int r = 0; r < rowsInA; r++) 
		{
			...
			for (int c = 0; c < columnsInB; c++) 
			{
	               ...
	               {
	                   C[r][c] = C[r][c] + a[r][k] * b[k][c]; //C[row][col] = # in row of A * corresponding # in col of B 
	               }
	           }
	       }
	       return C;
	...
	public static void main(String[] args) 
	{
	    ...
	    {
	    	System.out.println("Sorry, but in order for this to work, number of columns in A must match number of rows in B: ");
	    ...
	    int[][] result = multiply(A, B); //special method to multiply the 2 matrices
